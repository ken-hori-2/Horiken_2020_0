    ...
    def CamRgbImageCallback(self, rgb_image_data):
        ...
        if self.person_bbox.probability > 0.0 :
           ...
           # 一旦、BoundingBoxの中心位置の深度を取得 (今後改善予定）
            ...
            rospy.loginfo('Class : traffic light, Score: %.2f, Dist: %.2fm ' %(self.person_bbox.probability*100, m_person_depth))
            #text = "person " +('%dmm' % m_person_depth)

            text = "traffic light " +('%.2fm' % m_person_depth)
    ...
    def DarknetBboxCallback(self, darknet_bboxs):
        ...
        if len(bboxs) != 0 :
            for i, bb in enumerate(bboxs) :
                if bboxs[i].Class == 'traffic light' and bboxs[i].probability >= self.m_pub_threshold:
                    person_bbox = bboxs[i]        
